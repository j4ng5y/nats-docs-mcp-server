...
// NOTE: This test is skipped because it takes too long (5+ minutes) with real exponential backoff delays.
// The max delay cap behavior is validated by property tests instead.
func TestHTTPClientMaxDelayCap(t *testing.T) {
	t.Skip("Skipping test - takes too long (5+ minutes) with real exponential backoff delays")
	var attemptTimes []time.Time
